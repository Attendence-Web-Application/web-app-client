...
import { DELETE_STUDENT_CLASS_URL, DELETE_PROFESSOR_CLASS_URL, FIND_ALL_USER_URL, FIND_ROLL_CALL_BY_CLASS_URL } from '../utils/api';
...
const Classrooms = ({ classrooms, setClassrooms }) => {
    ...
    const handleDeleteClass = async (id) => {
        ...
        if (isStudent) { //delete in class_enroll
            ...
            try {
                const response = await fetch(FIND_ROLL_CALL_BY_CLASS_URL + id, {mode: "cors"});
                const data = await response.json();
                if (data.length == 0) {
            ...
            {isShow && <DialogWrapper>
              <Dialog open={isShow} onClose={!isShow} className="dialog_box">
                  <div style={{width: 500, margin: '0 auto'}}>
                      <DialogTitle className={classes.alignItemsAndJustifyContentTitle}>{"Can not drop class after first roll call"}</DialogTitle>
