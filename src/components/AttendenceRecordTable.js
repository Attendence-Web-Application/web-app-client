...
const AttendenceRecordTable = ({classNumber, classId}) => {
    ...
    const columns = [
        {id: 'Expire Time', label: 'Time', minWidth: 50},
        {id: 'Count', label: 'Count', minWidth: 50},
        {id: 'Rate', label: 'Rate', minWidht:50},
                                ...
                                return (
                                    <TableRow className="body_row" key={row.id}>
                                        {columns.map((column) => {
                                            ...
                                            else return (<TableCell className="table_cell"><button className='table_btn' onClick={() => setIsShow(true)}>Enter</button></TableCell>);
                                        })}
                                        <AttendanceDetailPage isShow = {isShow} setIsShow = {setIsShow} rollCallId={row.id}/>
                                    </TableRow>
                                )
                            }
                        )
