...
import { FIND_ALL_USER_URL, GET_ALL_CLASS, GET_ALL_TITLE, GET_USER_BY_TITLE } from '../utils/api'
...
const defaultClassNumber = "Defaute Class Number"
...
const InvitationForm = ({ handleSubmit, handleCancel }) => {
    ...
    const [classNumber, setClassNumber] = useState(defaultClassId)
    ...
    const fetchAllClass = async () => {
        try {
            ...
            // setClassroomList([""])
            // for (let i = 0; i < data.length; i++) {
            //     setClassroomList(prev => [...prev, ...data[i].title])
            // }
    ...
    const handleValidSubmit = (e) => {
        ...
        console.log(classId)
        // console.log(newCode)
        // console.log("-------------handle valid submit function--------------------")
        // handleSubmit(classId, "0")
    ...
    // const handleReset = (e) => {
    //     setClassroomList([defaultClass])
    //     setClassroom(defaultClass)
        ...
        <Wrapper>
            <form onSubmit={handleValidSubmit} className='add_box'>
                <div className='user_box'>
                    ...
                    {/* <select id="classroom" value={classroom} onChange={classroomChange} required>
                        {classList.map((classItem, index) => {
                            return <option value={classItem.title} key={index} disabled={classItem.title === defaultClass ? true : false}>{classItem.title}</option>
                        })}
                    </select> */}
                ...
                <div className='button_div'>
                    ...
                    {/* <button type="reset" className='form_btn reset' onClick={handleReset}>Reset</button> */}
